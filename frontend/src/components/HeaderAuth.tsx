    ...
    <div className="w-full bg-dark-800 border-b border-elysPink-600 shadow-lg">
      <div className="max-w-6xl mx-auto px-4 py-3 flex justify-between items-center">
        ...
        <div className="flex items-center space-x-3">
          {!hasToken ? (
            <>
              <Link className="text-elysViolet-400 hover:text-elysViolet-300 transition-colors duration-300 px-3 py-1 rounded-lg hover:bg-dark-700" href="/auth/login">{t('navigation.login')}</Link>
              <Link className="text-elysPink-400 hover:text-elysPink-300 transition-colors duration-300 px-3 py-1 rounded-lg hover:bg-dark-700" href="/auth/signup">{t('navigation.signup')}</Link>
            </>
          ) : (
            <>
              {isAdmin && (
              ...
              <Link href="/appointments-review" className="relative text-elysViolet-400 hover:text-elysViolet-300 transition-colors duration-300 px-3 py-1 rounded-lg hover:bg-dark-700 flex items-center space-x-1">
                ...
                <span className="hidden sm:inline">{t('navigation.review')}</span>
                {pendingCount > 0 && (
              ...
              <button onClick={handleLogout} className="text-elysBlue-400 hover:text-elysBlue-300 transition-colors duration-300 px-3 py-1 rounded-lg hover:bg-dark-700">{t('navigation.logout')}</button>
            </>
          )}
        </div>
      </div>
